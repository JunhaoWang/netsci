...
def p1(S, num_node, name, max_num_node = 10000):
    ...
    plt.title('degree distribution plot, slope: {}, bias: {}, time: {}'.format(str(round(slope, 2)), str(round(bias, 2)), elapased_time))
    ...
    plt.ylabel('log(count)')
    plt.title('local clustering distribution plot, local clustering mean: {}, time: {}'.format(str(round(clustering_mean, 2)),
                                                                                               elapased_time))
    ...
    plt.ylabel('log(count)')
    plt.title('all pair shortest path length distribution plot, shortest path length mean: {}, time : {}'.format(
    ...
    plt.title('top 20 laplacian eigenvalue distribution, eigengap: {}, time: {}'.format(str(round(eigen_gap, 2)), elapased_time))
    ...
    plt.title('assortativity plot, coef = {}, time: {}'.format(str(round(asso_coef, 2)), elapased_time))
    ...
    plt.scatter(x, y)
    plt.title('degree clustering plot, time: {}'.format(elapased_time))
    plt.xlabel('degree')
...
def p3(S, num_node):
    ...
    for i in tqdm(range(initial_num_node, initial_num_node + t)):
        m_ = truncated_normal(m, m // 2, 1, 2 * m)
        ns = np.random.choice(idx_ba, size=int(m_), p=degree_ba/np.sum(degree_ba), replace=False).tolist()
        for n in ns:
            degree_ba[n] = degree_ba[n] + 1
            S_ba[i,n] = 1
            S_ba[n,i] = 1
